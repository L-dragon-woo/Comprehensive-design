...
export function clearAuth() {
  localStorage.removeItem(accessKey); localStorage.removeItem(refreshKey); localStorage.removeItem(userKey)
  window.dispatchEvent(new CustomEvent("skinai:auth-updated"))
}
...
export async function logout() {
  ...
  if (token) await apiFetch("/api/auth/logout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refreshToken: token }) }).catch(() => undefined)
  clearAuth()
}
