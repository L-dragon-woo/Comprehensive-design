...
import type { ChatMessage, ChatReference } from "@/lib/skinai"
...
const evidenceReferences = {
  rejuran: [
    {
      title: "Comparison of polynucleotide and hyaluronic acid fillers on periocular rejuvenation",
      source: "PubMed · randomized, double-blind, split-face trial",
      url: "https://pubmed.ncbi.nlm.nih.gov/32248707/",
      summary: "폴리뉴클레오타이드(PN)와 히알루론산 필러를 비교해 피부 탄력과 수분 지표 변화를 평가한 연구예요.",
    },
    {
      title: "Water Retention Ability of the Dermis Following Polynucleotide Intradermal Injection",
      source: "PMC · pilot study",
      url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC12658717/",
      summary: "PN 피내 주입 후 피부 수분 분포, 탄력, 피부 두께 등 생체 지표 변화를 확인한 파일럿 연구예요.",
    },
  ],
  pico: [
    {
      title: "Use of Picosecond Laser for Melasma Treatment: A Narrative Review",
      source: "Photobiomodulation, Photomedicine, and Laser Surgery · review",
      url: "https://journals.sagepub.com/doi/abs/10.1089/photob.2023.0096",
      summary: "기미와 색소 병변에서 피코초 레이저 사용 근거와 한계를 정리한 리뷰 논문이에요.",
    },
    {
      title: "Efficacy and safety of picosecond laser for the treatment of melasma",
      source: "Lasers in Medical Science · systematic review and meta-analysis",
      url: "https://link.springer.com/article/10.1007/s10103-023-03744-y",
      summary: "피코초 레이저의 기미 치료 효과와 염증 후 과색소침착 같은 부작용 위험을 함께 분석한 연구예요.",
    },
  ],
  aquapeel: [
    {
      title: "Efficacy and Tolerability of HydraFacial Clarifying Treatment Series in Active Acne Vulgaris",
      source: "PubMed · multicenter open-label study",
      url: "https://pubmed.ncbi.nlm.nih.gov/36569524/",
      summary: "흡입, 필링, 세럼 적용을 포함한 하이드라페이셜 계열 관리가 여드름 피부 외관에 미치는 영향을 평가했어요.",
    },
    {
      title: "Evaluation of supramolecular salicylic acid followed by non-ablative fractional laser",
      source: "PubMed · clinical study",
      url: "https://pubmed.ncbi.nlm.nih.gov/36947275/",
      summary: "살리실산 기반 필링과 레이저 병합이 여드름, 모공, 피지 지표에 미치는 변화를 확인한 연구예요.",
    },
  ],
} satisfies Record<string, ChatReference[]>

function uniqueReferences(references: ChatReference[]) {
  return references.filter((reference, index, array) => array.findIndex((item) => item.url === reference.url) === index)
}
const input = ref("")
...
function getAIResponse(question: string): Pick<ChatMessage, "content" | "references"> {
  if (question.includes("쉽게 설명") || question.includes("추천된 시술")) {
    return {
      content: "추천 시술을 쉽게 정리해드릴게요.\n\n리쥬란 힐러는 건조함과 피부결 개선을 상담해볼 만하고, 피코토닝은 볼 부근 색소침착 완화 목적에 맞아요. 아쿠아필은 T존 유분과 모공 관리에 보조적으로 고려할 수 있어요.\n\n실제 시술 여부와 강도는 피부 민감도, 병력, 최근 시술 이력을 확인한 뒤 결정하는 것이 좋아요.",
      references: uniqueReferences([...evidenceReferences.rejuran, ...evidenceReferences.pico, ...evidenceReferences.aquapeel]),
    }
  }
...
function handleSend(messageText?: string) {
  ...
  window.setTimeout(() => {
    ...
    messages.value.push({ id: `${Date.now()}-ai`, role: "assistant", content: response.content, references: response.references, timestamp: new Date() })
